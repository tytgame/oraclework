...
-- 위의 구문을 실행하면 실제로는 아래와 같이 실행됨(서브쿼리)
-- 실제로 돌아가는 원리 = 서브쿼리
...
-- 실제 테이블에 (NYLL, 인턴) 추가. JOB_CODE는 NOT NULL이기 때문에 오류
...
-- 2개 동일 : 한 그룹에 여러명이 포함되어 있기 때문에
-- 4) 그룹함수나 GROUP BY절이 포함된 경우
-- 5) DISTINCT 구문이 포함된 경우(GROUP BY와 동일 : 한 그룹에 여러명)
...
-- INSERT (오류)
INSERT INTO VW_JOIN VALUES('400','이순신','981112-2838382','J1','총무부');
