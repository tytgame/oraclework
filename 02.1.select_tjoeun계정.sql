...
SELECT EMP_ID, EMP_NAME, SALARY, '원' AS 단위, BONUS, '%' AS "단위(%)"
FROM EMPLOYEE;
--------------------------------------------------------------------------------
/*
    <연결 연산자:||>
...
-- EMPLOYEE 테이블에서 직급코드의 중복을 제거한 데이터 조회
SELECT DISTINCT JOB_CODE
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 부서코드의 중복을 제거한 데이터 조회
...
-- EMPLOYEE 테이블에서 입사일이 '90/01/01 ~ 01/01/01' 인 사원들의 사번, 사원명, 입사일 조회
...
FROM EMPLOYEE
WHERE EMAIL LIKE '____%'; -- 언더바 4개로 인식 글자가 4글자 이상을 가져옴. 오류
/*
...
-- EMPLOYEE 테이블에서 부서배치를 받지 못했지만 보너스를 받는 사원들의 사번, 사원명, 부서코드 조회
...
-- EMPLOYEE 테이블에서 부서코드가 D6이거나 D8이거나 D9인 사원을 제외한 나머지 사원들의 사원명, 부서코드, 급여조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 직급코드가 'J7'이거나 'J2'인 직원들 중 급여가 200만원 이상인 사원들의 모든컬럼 조회
/*
SELECT *
FROM EMPLOYEE
