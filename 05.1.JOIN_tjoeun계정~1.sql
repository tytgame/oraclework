/*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 각각 테이블에 담고 있음
      (중복을 최소화하기 위해)
...
-- 해결방법 2) JOIN USING 구문 사용하는 방법(두 컬럼이 일치할 때만 사용가능)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
...
*/
-- 모든 사원명, 부서명, 급여, 연봉(부서를 배치받지 못한 사원도 조회)
...
-- 3) FULL [OUTER] JOIN : 두 테이블이 가진 모든 행 조화(오라클 전용구문은 사용 못함)
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE
...
/*
    *비등가 조인
    매칭시킬 컬럼에 대한 조건 작성시 '='(등호)를 사용하지 않는 JOIN
...
--                              사수사번, 사수명, 사수의 부서코드 (EMPLOYEE M 조회)
-->> 오라클 전용 구문
-- 사수가 있는 사원만 조회
...
--실습 문제--
-- 1.사번, 사원명, 부서명, 지역명, 국가명 조회(EMPLOYEE, DEPARTMENT, LOCATIONM, NATIONAL 조인)
-->> 오라클 구문
