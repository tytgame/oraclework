...
/*
    ...
    1) INSERT INTO 테이블명 VALUES(값1, 값2, 값3, ...)
       테이블의 모든 컬럼에 대한 값을 넣고자 할때 사용
       컬럼의 순서를 지켜야 한다.
       
       부족하게 값을 넣을 경우 -> not enough value
       값을 많이 넣을 경우 -> too many
*/
...
------------------------------------------------------------------------------------------------
/*
    2) INSERT INTO TABLE 테이블명(컬럼명1, 컬럼명2, 컬럼명3...) VALUES(값1, 값2, 값3 ...)
      : 테이블에 내가 선택한 컬럼에 대한 값만 INSERT 할때
      그래도 한 행 단위로 추가됨.
...
-- 300이상
CREATE TABLE EMP_UPSAL
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    FROM EMPLOYEE
    WHERE 1=0;

--300 이하
CREATE TABLE EMP_DOWNSAL
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    FROM EMPLOYEE
    WHERE 1=0;
...
WHEN SALARY < 3000000 THEN
    INTO EMP_DOWNSAL VALUES(EMP_ID, EMP_NAME, SALARY, BONUS)
WHEN SALARY >= 3000000 THEN
    INTO EMP_UPSAL VALUES(EMP_ID, EMP_NAME, SALARY, BONUS)
...
-- 조정연, 유하보, 문정보 직원들의 급여와 보너스를 문정보 사원과 동일하게 인상
UPDATE EMP_SALARY
SET (SALARY,BONUS) = (SELECT SALARY, BONUS
                        FROM EMP_SALARY
                        WHERE EMP_ID = '217')
...
-- EMPLOYEE_COPY 테이블의 ASIA 지역에서 근무하는 사원들의 보너스를 0.3으로 변경
SELECT EMP_ID
FROM EMPLOYEE_COPY
